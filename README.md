...
## 🏆 Against the Evaluation Rubrics

### ① Potential Impact & Uniqueness (20%)

This is the only open-source system that provides AI-powered safety routing for **49 Indian cities** using real, multi-source data. The screenshots above show it working correctly in Surat, Kolkata, and Nashik — cities that existing navigation apps treat identically to Bengaluru but have very different crime and infrastructure profiles. The architecture is designed to scale to all ~100 Indian cities with populations over 300,000 — just run the data pipeline for each.

### ② DS/Algorithm, AI Technique, Architectural Scalability (50%)

- **XGBoost + LightGBM ensemble** on 53 features — same proven model, now trained on India-wide data
- **Modified Dijkstra** with composite cost function — proven optimal, runs in 60–100ms per city
- **On-demand city loading** — only cities that are actually requested are loaded into RAM
- **Celery + Redis workers** — background tasks don't block the API
- **Graceful degradation** — system runs with any subset of data, output quality scales with data availability
- **City-agnostic pipeline** — adding a new city = one command, zero code changes
- **`numba` JIT** — 5–10× speedup on spatial feature queries for large city graphs

### ③ Code Quality (10%)

- Same clean layered architecture: `ingestion` → `ai` → `routing` → `api` → `frontend` → `workers`
- `evaluation/benchmark.py` provides reproducible cross-city benchmarks
- `test_cities.py` provides integration tests for route schema validation
- Graceful fallback logic is explicit and documented — no silent failures
- `health` endpoint for deployment monitoring

### ④ Model Evaluation & Testing (20%)

- Multi-city training validates the model generalizes beyond Bengaluru
- `evaluation/benchmark.py` measures safety gain across all cities
- `test_cities.py` integration tests verify routing works in all loaded cities
- Screenshots prove real-world routing in Surat, Kolkata, and Nashik
- Graceful degradation tested and documented — system behavior with partial data is predictable and explicit
...
*Python · FastAPI · React · Leaflet · XGBoost · LightGBM · OSMnx · NASA VIIRS · Celery · Redis · numba · NCRB*
